...
function BadgeEarnedAnnouncement({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/85 p-5" onClick={onDismiss}>
      <div className="relative w-full max-w-md rounded-3xl border border-[#c9a84c]/40 bg-[#0d1b2a] p-7 text-center shadow-2xl" onClick={(event) => event.stopPropagation()}>
        <button onClick={onDismiss} className="absolute right-4 top-4 text-[#8096aa]" aria-label="Close achievement announcement"><X size={20} /></button>
        <p className="text-xs font-semibold uppercase tracking-[.24em] text-[#c9a84c]">{payload.tier ? `Upgraded to ${payload.tier.toUpperCase()}` : "Achievement unlocked"}</p>
        <div className="mx-auto mt-6 flex h-48 w-48 items-center justify-center rounded-full bg-[#c9a84c] px-5 text-sm font-bold uppercase text-[#0a1520]">{badge.name}</div>
        <h2 className="mt-6 text-3xl font-bold text-white">{badge.name}</h2>
        <p className="mt-2 text-sm text-[#a6b5c2]">{badge.description}</p>
        <button onClick={onViewTrophyCase} className="mt-7 w-full rounded-xl border border-[#c9a84c] px-4 py-3 text-xs font-bold uppercase tracking-wider text-[#c9a84c]">View in Trophy Case</button>
      </div>
    </div>
  );
}
