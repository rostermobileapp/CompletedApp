...
interface EditDutyModalProps {
  ...
  duty: {
    ...
    name: string;
    icon: string;
    scope: 'single_game' | 'every_game';
  };
}

export default function EditDutyModal({ isOpen, onClose, teamId, duty }: EditDutyModalProps) {
  const [name, setName] = useState('');
  const [selectedIcon, setSelectedIcon] = useState<string>('');
  const [scope, setScope] = useState<'single_game' | 'every_game'>('every_game');
  const { toast } = useToast();

  // Populate form with existing duty data when modal opens
  useEffect(() => {
    if (isOpen && duty) {
      setName(duty.name);
      setSelectedIcon(duty.icon);
      setScope(duty.scope);
    }
  }, [isOpen, duty]);
  ...
  const updateDutyMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest('PUT', `/api/teams/${teamId}/duties/${duty.id}`, {
        name,
        icon: selectedIcon,
        scope,
      });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="sm:max-w-[600px]" data-testid="dialog-edit-duty">
        <DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-2 justify-end">
            ...
            <Button type="submit" disabled={updateDutyMutation.isPending} data-testid="button-update-duty">
              {updateDutyMutation.isPending ? 'Updating...' : 'Update Duty'}
            </Button>
          </div>
        </form>
      </DialogContent>
