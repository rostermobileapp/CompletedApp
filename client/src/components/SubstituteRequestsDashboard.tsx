...
export function SubstituteRequestsDashboard({ className, gameId }: SubstituteRequestsDashboardProps) {
  ...
  const RequestCard = ({ request, isPending = true }: { request: any; isPending?: boolean }) => (
    <Card 
      className="rounded-lg border shadow-sm transition-all border-border bg-[#e2e2e2] dark:bg-[#212121] text-[#212121] dark:text-[#ffffff]"
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            <Badge 
              ...
              className={
                request.status === 'pending' ? 'bg-[#000000] text-[#212121] dark:text-[#ffffff]' :
                request.status === 'approved' ? 'bg-gray-700 text-gray-100 dark:bg-gray-700 dark:text-gray-100' :
          ...
          <div className="flex items-center gap-3 p-3 bg-[#e2e2e2] dark:bg-[#212121] rounded-lg border border-[hsl(var(--hairline))] shadow-[var(--elev-rest)]">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                Skill: {(request.originalPlayer as any).skillLevel || '—'}
              </p>
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-3 p-3 bg-[#e2e2e2] dark:bg-[#212121] rounded-lg border border-[hsl(var(--hairline))] shadow-[var(--elev-rest)]">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">
                  Skill: {(request.substitutePlayer as any).skillLevel || '—'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3 pt-2 border-t border-border">
            ...
            <div className="flex gap-2">
              <Button
                ...
                className="bg-[#000000] text-[#fcfcfc] hover:bg-[#1a1a1a]"
              ...
              <Button
                ...
                disabled={updateRequestMutation.isPending}
                className="border-border hover:bg-[#1a1a1a]"
    ...
    return (
      <div className={className}>
        <div className="space-y-4">
          {[...Array(3)].map((_, i) => (
            <Card key={i} className="animate-pulse bg-[#1a1a1a] border-border">
              <CardHeader>
    ...
    <div className={className} data-testid="substitute-requests-dashboard">
      <div className="space-y-6">
        ...
        {processedRequests.length > 0 && (
          <div>
            {pendingRequests.length > 0 && <Separator className="bg-border" />}
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
