...
A mobile-optimized photo album feature allows tournament participants to upload, view, and download photos from tournament events. The feature includes a responsive photo grid layout, fullscreen viewer with swipe gesture navigation, pinch-to-zoom support, and download all photos as ZIP functionality. Access is restricted to approved tournament participants only. The Photos button is accessible from the home screen's quick access cards (replacing the Stats button) and navigates to the Photos tab in the tournament detail page. Backend validation enforces participant access, file type restrictions (JPEG, PNG, GIF, WebP), and 10MB file size limits. Photos are stored in Supabase Storage with proper cleanup on deletion.
