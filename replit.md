...
- **Opposing Team Filter Fix for Substitution Requests** (October 2025): Fixed critical security bug in player substitution system where the opposing team filter was incorrectly using the requester's team ID instead of the original player's team ID. This caused commissioners and multi-team players to see incorrect roster options when requesting substitutes. The fix propagates the original player's team context through the callback chain (RSVPDetailModal → GameDetails/Calendar → SubstituteRequestModal) using a new `originalPlayerTeamId` prop, ensuring the opposing team is always calculated based on the player being substituted, not the person making the request.
- **Same Team Player Filter Fix** (October 2025): Fixed critical bug where players from the SAME team as the original player were appearing in the substitution list. Updated SubstituteRequestModal filtering logic to exclude BOTH the original player's team AND the opposing team. Now only shows players from OTHER teams in the league (not playing in the current game), ensuring proper substitution requests.
- **Game Time Display Fix** (October 2025): Fixed 4-hour timezone difference bug in SubstituteRequestModal where game times were incorrectly converted between UTC and local timezones. Updated time display to use UTC methods (getUTCHours/getUTCMinutes) to extract time directly from ISO string without timezone conversion, ensuring times match between Calendar page and substitution modal.
...
- **Player Substitution Components Redesign** (October 2025): All player substitution request components redesigned with consistent dark theme matching the rest of the application. SubstituteRequestsDashboard now features dark card backgrounds (bg-[#1a1a1a]), dark borders (border-border), and dark input fields (bg-[#212121]). Player cards display skill levels instead of email addresses, with consistent styling across all sections. Badge colors updated: pending requests use black (bg-[#000000]), approved requests use gray (bg-gray-700), and denied requests use red (bg-red-600). All UI elements now seamlessly integrate with the app's dark theme for a cohesive user experience.
