...
For desktop browsers (viewport ≥1024px and not running inside a Capacitor/Natively wrapper), the app switches to a three-column desktop shell (`DesktopAppShell`): a 240px left primary sidebar (Home / My Team / Messages / Payments / Profile), a permanent 280px secondary menu column (`DesktopMenuColumn`) showing all 8 action items (Schedule Scrimmage, Invite Groups, Scrimmage Management, Create a Team, Create a League, League Management, Tournaments, Scorekeeper) with the same permission/lock logic as the mobile `SlideOutMenu`, and a wide main content area with a sticky team/league selector header. The desktop header also surfaces "Find a League / Tournament", "Find a Team", and "Send Feedback" buttons on the right side. Mobile (<1024px) and native wrappers continue to render the existing single-column layout with the slide-out menu behind a hamburger.
