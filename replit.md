...
- **Premium Feature Alert Dialog** (October 2025): Implemented user-friendly UI feedback for locked premium features. When free-tier users click on premium features in the slide-out menu, an elegant dialog appears with an animated GIF, informative text ("This is a Premium feature"), and a "Manage Subscription" button that navigates to the subscription page. This provides clear visual feedback and a smooth upgrade path for users attempting to access restricted features. The dialog integrates seamlessly with the slide-out menu's permission-gating system.
