...
export interface IStorage {
  ...
  createLeagueProGrant(grant: InsertLeagueProGrant): Promise<LeagueProGrant>;
  getLeagueProGrant(id: string): Promise<LeagueProGrant | undefined>;
   ...
   * members (in join order) until the grant is full. Used by the webhook
   * immediately after payment confirmation. Returns the number of newly
   * assigned seats.
   */
...
export class DatabaseStorage implements IStorage {
  ...
  private async migrateUserForeignKeys(oldId: string, newId: string): Promise<void> {
    await db.transaction(async (tx) => {
      ...
      await tx.execute(sql`DELETE FROM typing_indicators WHERE user_id = ${oldId}`);
      
      await tx.execute(sql`UPDATE league_memberships SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE team_memberships SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE player_stats SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE game_rsvps SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE game_goals SET scorer_id = ${newId} WHERE scorer_id = ${oldId}`);
      await tx.execute(sql`UPDATE game_goals SET primary_assist_id = ${newId} WHERE primary_assist_id = ${oldId}`);
      await tx.execute(sql`UPDATE game_goals SET secondary_assist_id = ${newId} WHERE secondary_assist_id = ${oldId}`);
      await tx.execute(sql`UPDATE game_penalties SET player_id = ${newId} WHERE player_id = ${oldId}`);
      await tx.execute(sql`UPDATE game_goalies SET goalie_user_id = ${newId} WHERE goalie_user_id = ${oldId}`);
      await tx.execute(sql`UPDATE game_stars SET first_star_user_id = ${newId} WHERE first_star_user_id = ${oldId}`);
      await tx.execute(sql`UPDATE game_stars SET second_star_user_id = ${newId} WHERE second_star_user_id = ${oldId}`);
      await tx.execute(sql`UPDATE game_stars SET third_star_user_id = ${newId} WHERE third_star_user_id = ${oldId}`);
      await tx.execute(sql`UPDATE game_stars SET awarded_by = ${newId} WHERE awarded_by = ${oldId}`);
      await tx.execute(sql`UPDATE game_score_submissions SET submitted_by = ${newId} WHERE submitted_by = ${oldId}`);
      await tx.execute(sql`UPDATE messages SET sender_id = ${newId} WHERE sender_id = ${oldId}`);
      ...
      await tx.execute(sql`UPDATE announcement_comments SET author_id = ${newId} WHERE author_id = ${oldId}`);
      await tx.execute(sql`UPDATE announcement_reactions SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE announcement_read_status SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE announcement_poll_votes SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE announcement_visibility SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE payment_requests SET creator_id = ${newId} WHERE creator_id = ${oldId}`);
      await tx.execute(sql`UPDATE payment_request_recipients SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE substitute_requests SET original_player_id = ${newId} WHERE original_player_id = ${oldId}`);
      await tx.execute(sql`UPDATE substitute_requests SET substitute_player_id = ${newId} WHERE substitute_player_id = ${oldId}`);
      await tx.execute(sql`UPDATE substitute_requests SET requested_by = ${newId} WHERE requested_by = ${oldId}`);
      await tx.execute(sql`UPDATE substitution_approvals SET approver_id = ${newId} WHERE approver_id = ${oldId}`);
      await tx.execute(sql`UPDATE scrimmages SET creator_id = ${newId} WHERE creator_id = ${oldId}`);
      await tx.execute(sql`UPDATE scrimmage_invites SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE scrimmage_requests SET player_id = ${newId} WHERE player_id = ${oldId}`);
      await tx.execute(sql`UPDATE scrimmage_co_hosts SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE scrimmage_co_hosts SET added_by = ${newId} WHERE added_by = ${oldId}`);
      await tx.execute(sql`UPDATE scrimmage_reminders_sent SET player_id = ${newId} WHERE player_id = ${oldId}`);
      await tx.execute(sql`UPDATE team_events SET creator_id = ${newId} WHERE creator_id = ${oldId}`);
      await tx.execute(sql`UPDATE team_event_rsvps SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE event_participants SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE event_reminders_sent SET player_id = ${newId} WHERE player_id = ${oldId}`);
      await tx.execute(sql`UPDATE rsvp_reminders_sent SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE personal_reminders SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE duty_assignments SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE duty_exclusions SET excluded_by = ${newId} WHERE excluded_by = ${oldId}`);
      await tx.execute(sql`UPDATE duty_templates SET created_by = ${newId} WHERE created_by = ${oldId}`);
      await tx.execute(sql`UPDATE feedback_submissions SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE tournaments SET created_by = ${newId} WHERE created_by = ${oldId}`);
      await tx.execute(sql`UPDATE tournament_participants SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE tournament_participants SET approved_by = ${newId} WHERE approved_by = ${oldId}`);
      await tx.execute(sql`UPDATE tournament_stats SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE tournament_match_rsvps SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE tournament_photos SET uploaded_by = ${newId} WHERE uploaded_by = ${oldId}`);
      await tx.execute(sql`UPDATE tournament_photo_tags SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE tournament_photo_tags SET tagged_by = ${newId} WHERE tagged_by = ${oldId}`);
      await tx.execute(sql`UPDATE league_photos SET uploaded_by = ${newId} WHERE uploaded_by = ${oldId}`);
      await tx.execute(sql`UPDATE league_photo_tags SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE league_photo_tags SET tagged_by = ${newId} WHERE tagged_by = ${oldId}`);
      await tx.execute(sql`UPDATE facility_memberships SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE drafts SET created_by = ${newId} WHERE created_by = ${oldId}`);
      await tx.execute(sql`UPDATE draft_picks SET player_id = ${newId} WHERE player_id = ${oldId}`);
      await tx.execute(sql`UPDATE calendar_events SET created_by = ${newId} WHERE created_by = ${oldId}`);
      await tx.execute(sql`UPDATE chat_poll_votes SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE invite_groups SET creator_id = ${newId} WHERE creator_id = ${oldId}`);
      await tx.execute(sql`UPDATE invite_group_members SET user_id = ${newId} WHERE user_id = ${oldId}`);
      await tx.execute(sql`UPDATE imported_players SET merged_with_user_id = ${newId} WHERE merged_with_user_id = ${oldId}`);
      await tx.execute(sql`UPDATE player_imports SET imported_by = ${newId} WHERE imported_by = ${oldId}`);
      await tx.execute(sql`UPDATE player_merge_requests SET existing_user_id = ${newId} WHERE existing_user_id = ${oldId}`);
      await tx.execute(sql`UPDATE player_merge_requests SET reviewed_by = ${newId} WHERE reviewed_by = ${oldId}`);
      await tx.execute(sql`UPDATE schedule_imports SET imported_by = ${newId} WHERE imported_by = ${oldId}`);
      await tx.execute(sql`UPDATE placeholder_players SET added_by = ${newId} WHERE added_by = ${oldId}`);
      await tx.execute(sql`UPDATE line_combination_assignments SET player_id = ${newId} WHERE player_id = ${oldId}`);
      await tx.execute(sql`UPDATE games SET home_beverage_duty_user_id = ${newId} WHERE home_beverage_duty_user_id = ${oldId}`);
      await tx.execute(sql`UPDATE games SET away_beverage_duty_user_id = ${newId} WHERE away_beverage_duty_user_id = ${oldId}`);
      await tx.execute(sql`UPDATE leagues SET commissioner_id = ${newId} WHERE commissioner_id = ${oldId}`);
      await tx.execute(sql`UPDATE teams SET captain_id = ${newId} WHERE captain_id = ${oldId}`);
      await tx.execute(sql`UPDATE teams SET creator_id = ${newId} WHERE creator_id = ${oldId}`);
      await tx.execute(sql`UPDATE team_league_requests SET requested_by = ${newId} WHERE requested_by = ${oldId}`);
      await tx.execute(sql`UPDATE team_league_requests SET approved_by = ${newId} WHERE approved_by = ${oldId}`);
      await tx.execute(sql`UPDATE league_memberships SET approved_by = ${newId} WHERE approved_by = ${oldId}`);
      await tx.execute(sql`UPDATE team_memberships SET approved_by = ${newId} WHERE approved_by = ${oldId}`);
    });
  ...
  async createLeagueProGrant(grant: InsertLeagueProGrant): Promise<LeagueProGrant> {
    const [row] = await db.insert(leagueProGrants).values(grant).returning();
    return row;
  }

  async getLeagueProGrant(id: string): Promise<LeagueProGrant | undefined> {
    const [row] = await db.select().from(leagueProGrants).where(eq(leagueProGrants.id, id)).limit(1);
    return row;
  }

  async getLeagueProGrantByStripeSession(sessionId: string): Promise<LeagueProGrant | undefined> {
    const [row] = await db
      .select()
      .from(leagueProGrants)
      .where(eq(leagueProGrants.stripeCheckoutSessionId, sessionId))
      .limit(1);
    return row;
  }

  async getLeagueProGrantsByLeague(leagueId: string): Promise<LeagueProGrant[]> {
    return db
      .select()
      .from(leagueProGrants)
      .where(eq(leagueProGrants.leagueId, leagueId))
      .orderBy(desc(leagueProGrants.createdAt));
  }

  async markLeagueProGrantPaid(id: string, paymentIntentId: string | null): Promise<LeagueProGrant> {
    const [row] = await db
      ...
      .where(eq(leagueProGrants.id, id))
      .returning();
    return row;
  }
  ...
  async getActiveLeagueProGrantsForLeague(leagueId: string, monthYM: string): Promise<LeagueProGrant[]> {
    return db
      ...
      .where(
        and(
          eq(leagueProGrants.leagueId, leagueId),
          eq(leagueProGrants.status, 'paid'),
          lte(leagueProGrants.startMonth, monthYM),
          gte(leagueProGrants.endMonth, monthYM),
        )
      )
      .orderBy(asc(leagueProGrants.createdAt));
  }

  async getLeagueProSeatsByGrant(grantId: string): Promise<LeagueProSeat[]> {
    return db
      .select()
      .from(leagueProSeats)
      .where(eq(leagueProSeats.grantId, grantId))
      .orderBy(asc(leagueProSeats.assignedAt));
  }
  ...
  async backfillLeagueProSeats(grantId: string): Promise<number> {
    ...
    const existingSeats = await this.getLeagueProSeatsByGrant(grantId);
    const remaining = grant.seatCount - existingSeats.length;
  ...
  async approveLeagueMembership(membershipId: string, approverId: string): Promise<LeagueMembership> {
    ...
    // Auto-assign a Player Pro seat from any active League-Wide Pro grant the
    // commissioner has paid for. The grant's seatCount cap is enforced inside
    // assignLeagueProSeat — when grants are full, this is a no-op (player N+1
    // simply remains free-tier) and we notify them so they understand why.
    try {
      const { currentMonth } = await import('./leaguePro');
      const monthYM = currentMonth();
      const activeGrants = await this.getActiveLeagueProGrantsForLeague(
